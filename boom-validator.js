...
function BoomValidation(options) {
    ...
    if (!this.options['error-message-class-name']) {
        this.options['error-message-class-name'] = DEFAULTS['error-message-class-name']
    }

    this.makeReadable = (text, delimiter) => text.replace(delimiter, ' ').replace(/\b\w/g, l => l.toUpperCase());

    this.parseErrorMessage = (elem, errorMessage) => this.makeReadable(errorMessage.replace(":field", elem.getAttribute('name')), '_');
    ...
    this.showErrorMessage = (elem, errorMessage) => {
        ...
        if (error_message) {
            ...
            helpBlock.className = this.option['custom-error-message-class-name'] || this.options['error-message-class-name'];
    ...
    this.hideErrorMessage = (elem) => {
        ...
        let errorDiv = elem.parentNode.querySelectorAll(
            '.' + errorDivSelector
        ).forEach(ele => {
            if (elem && ele) {
                ele.innerHTML = '';
    ...
    this.numeric = (elem, message = DEFAULTS.validation.rules.messages['numeric']) => {
    ...
    if (this.options.validation) {
        for (let i = 0; i < this.options.validation.length; i++) {
            ...
            if (currentOption['custom-error-message-class-name']) {
                this.option['custom-error-message-class-name'] = currentOption['custom-error-message-class-name'];
            }
